...
const index = async (req, res) => {
  try {
    const rows = await db.LearningMaterial.findAll();
...
const show = async (req, res) => {
  try {
    const item = await db.LearningMaterial.findByPk(req.params.id);
...
const store = async (req, res) => {
  try {
    const item = await db.LearningMaterial.create(req.body);
